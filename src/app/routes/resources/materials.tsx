...
import { Card, Pill, Icons, Icon, Money, NEX, Btn, nexAlpha } from '@/lib/nex'
...
export function MaterialsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return materials.filter((m) =>
      (m.filamentType?.name ?? '').toLowerCase().includes(q) ||
  ...
  useEffect(() => {
    ...
    if (!materialTypes.some((item) => item.id === filamentTypeId)) setFilamentTypeId('')
  }, [materialTypes, filamentTypeId])

  const parseOptionalNumber = (v: string) => { if (!v.trim()) return undefined; const n = Number(v); return Number.isFinite(n) ? n : undefined }
  const parseOptionalInt = (v: string) => { const n = parseOptionalNumber(v); return n !== undefined ? Math.trunc(n) : undefined }
  ...
  const getDefaultWeight = (material?: Material | null) => material?.materialType === 'FILAMENT' ? Math.max(1, Math.trunc(Number(material.spoolWeightG) || 1000)) : 1000
  ...
  const buildPayload = () => {
    const payload: Record<string, unknown> = { materialType, filamentTypeId: filamentTypeId || undefined, brandId: brandId || undefined, notes: notes.trim() || undefined }
    ...
    if (materialType === 'RESIN') { if (nextDensity !== undefined) payload.densityKgM3 = Math.max(1, nextDensity) }
    return payload
  }
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { toast.success('Matéria-prima removida'); setPendingDelete(null); queryClient.invalidateQueries({ queryKey: ['materials'] }) },
  ...
  const buildCreateStockPayload = () => {
    const p: Record<string, unknown> = { initialWeightG: Math.max(1, parseOptionalInt(initialWeightG) ?? 1), costPerKg: Math.max(0, costPerKg || 0), lotNumber: lotNumber.trim() || undefined, purchaseDate: purchaseDate || undefined, openedDate: openedDate || undefined, status: stockStatus }
    if (selectedStockMaterial?.materialType === 'FILAMENT') { p.color1Id = color1Id || undefined; p.color2Id = color2Id || undefined; p.color3Id = color3Id || undefined }
    return p
  }

  const buildUpdateStockPayload = () => {
    const p: Record<string, unknown> = { status: stockStatus, currentWeightG: Math.max(0, parseOptionalInt(currentWeightG) ?? 0), costPerKg: Math.max(0, costPerKg || 0) }
    if (lotNumber.trim()) p.lotNumber = lotNumber.trim()
    if (openedDate) p.openedDate = openedDate
    if (selectedStockMaterial?.materialType === 'FILAMENT') { p.color1Id = color1Id || null; p.color2Id = color2Id || null; p.color3Id = color3Id || null }
    return p
  }

  const saveStock = useMutation({
    mutationFn: (p: { materialId: string; stockId?: string }) =>
      p.stockId ? materialsService.updateStock(p.stockId, buildUpdateStockPayload()) : materialsService.addStock(p.materialId, buildCreateStockPayload()),
    onSuccess: (_, p) => {
  ...
  const closeStock = useMutation({
    ...
    onSuccess: () => {
      toast.success('Recipiente encerrado'); setPendingCloseStock(null)
  ...
  const canSubmitStock = !!stockMaterialId && (
    editingStockId ? Number.isFinite(Number(currentWeightG)) && Number(currentWeightG) >= 0 : Number.isFinite(Number(initialWeightG)) && Number(initialWeightG) > 0
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 h-9 px-3 rounded-md flex-1 max-w-xs" style={{ background: NEX.surface, border: `1px solid ${NEX.border}` }}>
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar matéria-prima..." className="bg-transparent flex-1 text-[12.5px] focus:outline-none" style={{ color: NEX.text }} />
          {search && <button onClick={() => setSearch('')} style={{ color: NEX.textMute }}><Icon d={Icons.x} size={12} /></button>}
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[11.5px]" style={{ color: NEX.textMute }}>{filtered.length} {filtered.length === 1 ? 'material' : 'materiais'}</span>
          <Btn kind="primary" size="md" icon={Icons.plus} onClick={openCreate}>Nova matéria-prima</Btn>
        </div>
      </div>
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7}>
                <div className="flex flex-col items-center justify-center py-14" style={{ color: NEX.textMute }}>
                  <Icon d={Icons.spool} size={28} style={{ marginBottom: 10, opacity: 0.4 }} />
                  <p className="text-[13px]">{search ? `Nenhum resultado para "${search}"` : 'Nenhuma matéria-prima cadastrada.'}</p>
                  {!search && <Btn kind="ghost" size="sm" icon={Icons.plus} className="mt-4" onClick={openCreate}>Adicionar primeira matéria-prima</Btn>}
                </div>
              </td></tr>
            )}
            {filtered.map((material) => (
              <tr key={material.id} className="group" style={{ borderTop: `1px solid ${NEX.border}` }}>
                <td className="px-4 py-3">
                  <div className="font-medium" style={{ color: NEX.text }}>{material.filamentType?.name ?? 'Sem tipo'}</div>
                ...
                <td className="px-4 text-right">
                  <div className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <ActionBtn color={NEX.cyan} title="Editar" onClick={() => openEdit(material)}><Icon d={Icons.edit} size={12} /></ActionBtn>
                    <ActionBtn color={NEX.red} title="Excluir" onClick={() => setPendingDelete(material)}><Icon d={Icons.trash} size={12} /></ActionBtn>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {/* Stock management panel — full width, shown when a material is selected */}
      {selectedStockMaterial && (
        <Card>
          <div className="flex items-center justify-between mb-3">
            <div>
              <div className="text-[13px] font-semibold">Gestão de {selectedStockMaterial.materialType === 'FILAMENT' ? 'bobinas' : 'potes de resina'}</div>
              <div className="text-[11px]" style={{ color: NEX.textDim }}>
                {selectedStockMaterial.filamentType?.name ?? 'Matéria-prima sem tipo'} · {selectedStockMaterial.brand?.name ?? 'Sem marca'}
              </div>
            </div>
            <button onClick={() => setStockMaterialId(null)} className="text-[11px]" style={{ color: NEX.textMute }}><Icon d={Icons.x} size={14} /></button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              <div className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: NEX.textMute }}>
                {editingStockId ? 'Alterar recipiente' : (selectedStockMaterial.materialType === 'FILAMENT' ? 'Nova bobina' : 'Novo pote')}
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                <ResourceField label="Status">
                  <select value={stockStatus} onChange={(e) => setStockStatus(e.target.value as MaterialStock['status'])} className={resourceInputCls}>
                    <option value="SEALED">Lacrado</option><option value="IN_USE">Em uso</option><option value="EMPTY">Vazio</option><option value="EXPIRED">Expirado</option>
                  </select>
                </ResourceField>
              </div>
              {editingStockId ? (
              ...
              {selectedStockMaterial.materialType === 'FILAMENT' && (
                <>
                  <div className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: NEX.textMute }}>Cores da bobina</div>
                  <div className="grid grid-cols-3 gap-2">
                    {(['Cor principal *', 'Cor secundária', 'Cor terciária'] as const).map((label, i) => {
                      ...
                      const val = vals[i]; const set = setters[i]; const others = vals.filter((_, j) => j !== i)
                      return (
                        <ResourceField key={label} label={label}>
                          <select value={val} onChange={(e) => set(e.target.value)} className={resourceInputCls}>
                            ...
                            {colors.map((c) => <option key={c.id} value={c.id} disabled={c.id !== val && others.includes(c.id)}>{c.name}</option>)}
                          </select>
                        </ResourceField>
                      )
                    })}
              ...
              <div className="flex gap-2">
                {editingStockId && <Btn kind="ghost" size="md" className="flex-1 justify-center" onClick={() => resetStockForm(selectedStockMaterial)}>Cancelar</Btn>}
            ...
            <div className="col-span-2">
              <div className="text-[10px] font-semibold uppercase tracking-widest mb-3" style={{ color: NEX.textMute }}>Recipientes cadastrados · {stocks.length}</div>
              {stocks.length === 0 ? (
                <div className="text-[12px] py-4" style={{ color: NEX.textMute }}>Nenhum recipiente cadastrado.</div>
              ) : (
                <table className="w-full text-[11.5px]">
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${NEX.border}` }}>
                      {['Recipiente', ...(selectedStockMaterial.materialType === 'FILAMENT' ? ['Cor(es)'] : []), 'Status', 'Peso / nível', 'Custo/kg', ''].map((h) => (
                        <th key={h} className="pb-2 text-left font-medium" style={{ color: NEX.textDim }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {stocks.map((stock) => {
                      ...
                      return (
                        <tr key={stock.id} className="group" style={{ borderTop: `1px solid ${NEX.border}` }}>
                          ...
                          <td className="py-2 text-right">
                            <div className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <ActionBtn color={NEX.cyan} title="Alterar" onClick={() => startEditStock(stock)}><Icon d={Icons.edit} size={12} /></ActionBtn>
                              <ActionBtn color={NEX.red} title="Encerrar" onClick={() => setPendingCloseStock(stock)}><Icon d={Icons.trash} size={12} /></ActionBtn>
                            </div>
                          </td>
                        </tr>
                      )
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title={editingId ? 'Editar matéria-prima' : 'Nova matéria-prima'}>
      ...
      <ConfirmDialog open={!!pendingDelete} onOpenChange={(o) => { if (!o) setPendingDelete(null) }} title="Excluir matéria-prima?"
        description={pendingDelete ? `Esta ação removerá permanentemente "${pendingDelete.filamentType?.name ?? pendingDelete.id.slice(0, 8)}".` : ''}
        confirmLabel="Excluir" variant="danger" loading={remove.isPending}
        onConfirm={() => { if (pendingDelete) remove.mutate(pendingDelete.id) }} />

      <ConfirmDialog open={!!pendingCloseStock} onOpenChange={(o) => { if (!o) setPendingCloseStock(null) }}
        title={pendingCloseMaterial?.materialType === 'FILAMENT' ? 'Encerrar bobina?' : 'Encerrar pote de resina?'}
        description={pendingCloseStock ? `Marca "${pendingCloseStock.lotNumber ?? pendingCloseStock.id.slice(0, 8)}" como vazio e ajusta peso para zero.` : ''}
        confirmLabel="Encerrar" variant="danger" loading={closeStock.isPending}
        onConfirm={() => { if (pendingCloseStock) closeStock.mutate(pendingCloseStock.id) }} />
    </div>
  )
}
